...
export class WETHGatewayService extends BaseService {
    constructor(provider, erc20Service, wethGatewayAddress) {
        ...
        this.generateRepayEthTxData = ({ lendingPool, amount, user, onBehalfOf, }) => {
    ...
    async borrowETH({ lendingPool, user, amount, debtTokenAddress, referralCode, }) {
        const txs = [];
        const convertedAmount = valueToWei(amount, 18);
        if (!debtTokenAddress) {
            throw new Error(`To borrow ETH you need to pass the variable WETH debt Token Address`);
        }
        ...
        const wethGatewayContract = this.getContractInstance(this.wethGatewayAddress);
        const txCallback = this.generateTxCallback({
            rawTxMethod: async () => wethGatewayContract.populateTransaction.borrowETH(lendingPool, convertedAmount, referralCode !== null && referralCode !== void 0 ? referralCode : '0'),
            from: user,
        });
        txs.push({
            tx: txCallback,
    ...
    repayETH({ lendingPool, user, amount, onBehalfOf }) {
        const convertedAmount = valueToWei(amount, 18);
        const wethGatewayContract = this.getContractInstance(this.wethGatewayAddress);
        const txCallback = this.generateTxCallback({
            rawTxMethod: async () => wethGatewayContract.populateTransaction.repayETH(lendingPool, convertedAmount, onBehalfOf !== null && onBehalfOf !== void 0 ? onBehalfOf : user),
            gasSurplus: 30,
            from: user,
            value: convertedAmount,
        });
